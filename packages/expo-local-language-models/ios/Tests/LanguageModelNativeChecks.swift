...
@testable import ExpoLocalLanguageModels
#endif
...
internal enum LanguageModelNativeChecks {
  ...
  static func bridge() async throws {
    ...
    defer { fixture.close() }
    try fixture.evaluate("expo.modules.ExpoLocalLanguageModels.getAvailabilityAsync(['en'], 'en').then(value => { globalThis.availability = JSON.parse(value); });")
    ...
    try fixture.evaluate("""
      expo.modules.ExpoLocalLanguageModels.createSessionAsync('{}').then(session => {
        globalThis.session = session;
        checks.push(session instanceof expo.modules.ExpoLocalLanguageModels.LanguageModelSession);
  ...
  static func resultAcceptanceBridge() async throws {
    ...
    try fixture.evaluate("""
      ...
      expo.modules.ExpoLocalLanguageModels.createSessionAsync('{}').then(async session => {
        globalThis.session = session;
  ...
  static func idleSessionAfterModuleTeardown() async throws {
    ...
    defer { fixture.close() }
    try fixture.evaluate("""
      expo.modules.ExpoLocalLanguageModels.createSessionAsync('{}')
...
private final class BridgeFixture {
  ...
  init(backend: any LanguageModelBackend = BridgeBackend()) {
    ...
    context = AppContext()
    installRuntime()
    context.moduleRegistry.register(module: LocalLanguageModelsModule(appContext: context, backend: backend), name: nil)
  }
  ...
  func destroyModule() { context.moduleRegistry.unregister(moduleName: "ExpoLocalLanguageModels") }
