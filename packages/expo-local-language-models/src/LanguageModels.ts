...
export async function getAvailabilityAsync(
  ...
  if (!ExpoLocalLanguageModels) return { status: 'unavailable', reason: 'module-unavailable' };
  let result: unknown;
  try {
    result = JSON.parse(
      await ExpoLocalLanguageModels.getAvailabilityAsync(
...
export async function prepareAsync(
  ...
  const nativeModule = ExpoLocalLanguageModels;
...
async function openSessionAsync(
  ...
  operation?.check();
  if (availability.status !== 'available' || !ExpoLocalLanguageModels) {
    throw new LanguageModelError(
