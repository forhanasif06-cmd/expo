...
/**
 * Extracts an embedded asset out of the APK into the given file. Supplied by callers that hold a
 * `Context`, so the downloader needs no APK access of its own.
 */
...
class FileDownloader(
  ...
  private fun preparePatchBaseAsset(
    ...
    // With embedded asset copying off, the launch asset row points into the APK instead of at a file
    // in the updates directory. BSPatch needs a real path, so the asset is extracted first.
  ...
  private fun extractEmbeddedPatchBaseAsset(
    ...
    // The embedded asset and resource fields are derived from the relative path rather than stored,
    // and the launch asset row is loaded without them.
    AndroidResourceAssetUtils.fillEmbeddedAssetIdentity(launchAssetEntity)
    if (launchAssetEntity.embeddedAssetFilename == null &&
      (launchAssetEntity.resourcesFolder == null || launchAssetEntity.resourcesFilename == null)
    ) {
      throw IOException("Could not read an embedded asset location from the launch asset path ${launchAssetEntity.relativePath}")
    }
