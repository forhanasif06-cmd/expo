...
function transformListProps(
  props: Omit<ListProps, 'children'> & Pick<NativeListProps, 'rowModifiers'>
): Omit<NativeListProps, 'children'> {
  const { modifiers, rowModifiers, ...restProps } = props;
  const eventModifiers = [...(modifiers ?? []), ...(rowModifiers ?? [])];
  return {
    modifiers,
    rowModifiers,
    ...(eventModifiers.length ? createViewModifierEventListener(eventModifiers) : undefined),
    ...restProps,
...
export interface ListDataProps<T> extends CommonViewModifierProps {
  ...
  /** Modifiers applied to every native row, including unmounted placeholders. */
  rowModifiers?: CommonViewModifierProps['modifiers'];
   ...
   * Setting this callback to undefined resets the notification, e.g. while refreshing.
   */
  onEndReached?: () => void;
  /**
...
function DataList<T>({
  ...
  useEffect(() => {
    if (lastKey === undefined || !onEndReached) {
      notifiedEnd.current = undefined;
      return;
    }
    if (!endAppeared || notifiedEnd.current === lastKey) return;
