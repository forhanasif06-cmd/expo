...
/**
 * Derives a stable ASCII identifier from an app name, for use as an Xcode project
 * name and as the `HelloWorld` replacement in templates.
 *
 * Symbols carry no name information and are dropped ('A & B' -> 'AB', 'Expo®' ->
 * 'Expo'). Accented and compatibility letters keep their base form ('Árbók' ->
 * 'Arbok', 'ﬁre' -> 'fire', fullwidth 'Ｅｘｐｏ' -> 'Expo'), and letters that NFKD
 * cannot decompose are transliterated ('Bjørn' -> 'Bjorn', 'Straße' -> 'Strasse').
 * A symbol-only name falls back to a slugify of the full name ('♥' -> 'love'), then
 * to 'app'.
 */
export function sanitizedName(name: string) {
  // NFKD before slugify, or letters are lost: 'Ċ' decomposes to 'C' + a combining
  // mark that the next line drops, while slugify deletes it outright.
