 ...
 * @deprecated To be removed in SDK 60
 */
interface ExternalCssInfo {
  href: string;
  /** Media query baked into the `<link>` tag, when present. */
  media?: string;
}

export function normalizeCssAssets(...assets: (AssetInfo | undefined)[]): CssAsset[] {
  const css = assets.flatMap((asset) =>
    (asset?.css ?? []).map(
  ...
  const externalCss = assets.flatMap((asset) =>
    (asset?.externalCss ?? []).map(
