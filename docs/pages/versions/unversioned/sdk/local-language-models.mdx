...
sourceCodeUrl: 'https://github.com/expo/expo/tree/main/packages/expo-local-language-models'
packageName: 'expo-local-language-models'
...
<APIInstallSection cmd={['$ npx expo install expo-local-language-models']} />
...
The plugin writes these Android settings during [Prebuild](/workflow/continuous-native-generation/). Installing or importing `expo-local-language-models` does not change native build settings by itself. These build requirements do not establish device support for Gemini Nano; check model availability at runtime.
...
```ts
import { generateAsync } from 'expo-local-language-models';
...
```ts
import { categorizeAsync, summarizeAsync } from 'expo-local-language-models';
...
```ts
import { generateAsync, schema } from 'expo-local-language-models';
...
```ts
import { generateAsync, schema, type ToolDefinition } from 'expo-local-language-models';

const categoryInput = schema.object({ category: schema.enum(['work', 'personal']) });
...
```ts
import { generateAsync, type ToolCall, type ToolDefinition } from 'expo-local-language-models';
...
Pass local image files in `images`. Import predefined tool objects from `expo-local-language-models/apple` to read text, barcodes, or QR codes:

```ts
import { generateAsync, schema } from 'expo-local-language-models';
import { Tools } from 'expo-local-language-models/apple';
...
export async function readReceipt(uri: string) {
  return await generateAsync('Read the receipt image and return its total.', {
    ...
    schema: schema.object({
      total: schema.string({ description: 'The total, including the currency shown.' }),
    }),
  });
}
```
...
export async function suggestTitleWithUsage(notes: string) {
  const result = await generateAsync(notes, { instructions: 'Suggest a short title.' });
  return {
    title: result.value,
...
```ts
import { getAvailabilityAsync } from 'expo-local-language-models';
...
```ts
import { generateAsync, LanguageModelError } from 'expo-local-language-models';
...
```ts
import { prepareAsync } from 'expo-local-language-models';
...
```ts
import { createSessionAsync } from 'expo-local-language-models';

export async function refineTitle(notes: string) {
  const session = await createSessionAsync({ instructions: 'Suggest concise note titles.' });
...
```ts
import * as LocalLanguageModels from 'expo-local-language-models';
```

<APISection packageName="expo-local-language-models" apiName="LocalLanguageModels" />
